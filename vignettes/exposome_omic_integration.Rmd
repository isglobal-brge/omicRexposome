...
The association studies are performed using the method `association`. This function requires, at last four, augments:
...
The argument `formula` should follow the pattern: `~sex+age`. The function `association` will fill the exposures in the `ExposomeSet` between `~` and the covariates `sex+age`.

`association` implements the `limma` pipeline (`lmFit` and `eBayes`) by taking care of the missing data in exposures, outcomes and omics data and locating and filtering common samples. The argiment `method` allows to select the fitting method used in `lmFit`. By default it takes the value `"ls"`  for least squares but it can also takes `"robust"` for robust regression.
...
Methylation data is encapsulated in a `GenomicRatioSet` object. In otehr to perform the association analysis between the methylation and exposome we encapsulate both data-sets into a `MultiDataSet` object.
...
The association studies between exposures and transcriptome are done in the same way that the ones with methylome. The function used is `association`, that takes as input an object of `MultiDataSet` class.
...
```{r gexp_tables}
cbind(
    Hits=tableHits(gexp, th=0.001)[ , -1],
...
The association analysis between exposures and proteome is also done using `association` method but avoiding the call to `eBayes` since we only have 47 features. The `eBayes` call can be avoided setting the argument `ebayes` to `FALSE`:
...
prot <- association(mds, formula=~Sex+Age, ebayes=FALSE,
...
cr_mcia <- crossomics(mds, method = "mcia" )
cr_mcia
```
...
cr_mcca <- crossomics(mds, method = "mcca", permute=c(5, 3))
cr_mcca
```
