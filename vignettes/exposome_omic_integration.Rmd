...
```{r methy_data, cache=FALSE, message=FALSE}
...
As can be seen, `crossomics` returns an object of class `ResultSet`. In the integration process, the different data-sets are subseted by common samples. This is done takeing advantadge of `MultiDataSet` capabilies. 
...
plotIntegration(cr_pe_mcia, colors=colors)
```
...
plotIntegration(cr_pe_mcca, colors=colors)
```
